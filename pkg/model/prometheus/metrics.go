...
type MetricData struct {
	MetricType   string `json:"result_type,omitempty" description:"result type, one of matrix, vector" csv:"metric_type"`
...
func ParseQueryRangeResp(value model.Value, metricFilter func(metric model.Metric) bool) MetricData {
	...
	for _, v := range data {
		...
		for k, val := range v.Metric {
			mv.Metadata[string(k)] = string(val)
		}

		for _, val := range v.Values {
			mv.Series = append(mv.Series, Point{float64(val.Timestamp), float64(val.Value)})
		}
...
func ParseQueryResp(value model.Value, metricFilter func(metric model.Metric) bool) MetricData {
	...
	for _, v := range data {
		...
		mv.Sample = &Point{float64(v.Timestamp), float64(v.Value)}
		res.MetricValues = append(res.MetricValues, mv)
	}
...
func Parse2pbQueryRangeResp(value model.Value, metricFilter func(metric model.Metric) bool) *pb.MetricsData {
	res := &pb.MetricsData{ResultType: MetricTypeMatrix}
	...
	for _, v := range data {
		...
		for k, val := range v.Metric {
			mv.Metric[string(k)] = string(val)
		}

		for _, val := range v.Values {
			mv.Values = append(mv.Values, &pb.Point{Timestamp: float64(val.Timestamp), Value: float64(val.Value)})
		}

		res.Result = append(res.Result, mv)
	}
...
func Parse2pbQueryResp(value model.Value, metricFilter func(metric model.Metric) bool) *pb.MetricsData {
	res := &pb.MetricsData{ResultType: MetricTypeVector}
	...
	for _, v := range data {
		...
		mv.Value = &pb.Point{Timestamp: float64(v.Timestamp), Value: float64(v.Value)}
		res.Result = append(res.Result, mv)
	}
