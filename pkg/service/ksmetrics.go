...
func (svc KsMetricsService) MetricsPlugins(name, status, sortBy string, limit, page string) ([]byte, error) {
	R := svc.kapiCli.RestyClient.R()
	R.SetQueryParams(parseDeploymentQuery(name, status, sortBy, limit, page))
...
func (svc KsMetricsService) PluginPods(plugin string) ([]byte, error) {
	...
	log.Debug(res.Request.RawRequest)
...
func parseDeploymentQuery(name, status, sortBy, limit, page string) map[string]string {
